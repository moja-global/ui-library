<template>
    <div class="calendar">
        ...
        <div class="calendar-layout"
            ...
            <div v-if="!year" class="calendar-body">
                <div class="calendar-row"
                    ...
                    <div class="calendar-day"
                         :class="{disabled: isDisabled(cell), selected: (!isDisabled(cell) && isSelected(cell)), activecell:!isDisabled(cell)}"
            ...
            <div v-if="year" class="calendar-body">
                <div class="calendar-row" v-for="(row,index) in generateYears" :key="'year'+index">
    ...
    export default {
        ...
        computed: {
            monthYearFormatted()
            {
                ...
                return this.monthes[ this.activeDate.substr( 4,2 ) ] + ' ' + this.selectedYear;
            },
            cellHeightUnit()
            {
        ...
        setup()
        {
            ...
            {
                return ( parseInt( month ) == 1 ) ? '12' : ( ( "0" + ( parseInt( month ) - 1 ) ).slice( -2 ) );
            }
            function getNextMonth( month )
            {
                return ( parseInt( month ) == 12 ) ? '01' : ( ( "0" + ( parseInt( month ) + 1 ) ).slice( -2 ) );
            }
            ...
            {
                if ( month == '01' )
                    return new Date( year - 1,parseInt( getPreviousMonth( month ) ),0 ).getDate();
                else
            ...
            function getNumberOfDaysInNextMonth()
            {
                if ( getNextMonth( getCurrentMonth() ) == '12' )
                    return new Date( getCurrentYear() + 1,parseInt( getNextMonth( getCurrentMonth() ) ),0 ).getDate();
                else
                    return new Date( getCurrentYear(),parseInt( getNextMonth( getCurrentMonth() ) ),0 ).getDate();
            }
            ...
            function selectDay( cell )
            {
                if ( !isDisabled( cell ) )
                {
                    ...
                    date.value = ( "0" + cell.day ).slice( -2 ) + '/' + cell.month + '/' + cell.year;
                    active.value = false;
                }
            }
            function selectYear( cell )
            {
                ...
                year.value = false;
                this.populateCalendar( activeDate.value.substr( 0,4 ),activeDate.value.substr( 4,6 ) );

            }
            function isSelected( cell )
            {
                if ( ( "0" + cell.day ).slice( -2 ) + cell.month + cell.year == activeDay.value )
                    return true;
            }
            function isSelectedYear( cell )
            {
                if ( cell == selectedYear.value )
                    return true
            }
        ...
        mounted()
        {
            ...
            this.populateCalendar( this.activeDate.substr( 0,4 ),this.activeDate.substr( 4,6 ) );
    ...
    .calendar .calendar-layout .calendar-body {
        ...
        .calendar .calendar-layout .calendar-body .calendar-row {
            ...
            .calendar .calendar-layout .calendar-body .calendar-row .calendar-day {
                ...
                .calendar .calendar-layout .calendar-body .calendar-row .calendar-day.calendar-year {
                    width: calc(100% / 5);
            ...
            .calendar .calendar-layout .calendar-body .calendar-row .activecell.calendar-day:hover {
                color: #475348;
            }

                .calendar .calendar-layout .calendar-body .calendar-row .activecell.calendar-day:hover:after {
                    width: 100%;
                }
